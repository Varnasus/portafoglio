...
import { Save, Download, RotateCcw, Share2, Wifi, WifiOff, Bot, User, Send, FileText, Grid3X3, Copy } from "lucide-react"
...
export function DiscoveryCanvas() {
  ...
  const isValidCanvasData = (obj: unknown): obj is CanvasData => {
    if (!obj || typeof obj !== 'object') return false
    const requiredKeys: (keyof CanvasData)[] = [
      'problemStatement', 'targetUsers', 'userJourney', 'keyAssumptions',
      'validationExperiments', 'successMetrics', 'technicalConstraints',
      'ethicalConsiderations', 'modelSelection', 'dataRequirements',
      'evaluationFramework', 'timeline'
    ]
  ...
  const isValidConversation = (arr: unknown): arr is ConversationMessage[] => {
  ...
  // Load data from sessionStorage on mount with validation
  useEffect(() => {
    if (!isHydrated) return

    try {
      // Check for URL parameters first (shared canvas)
      ...
      if (urlData) {
        try {
          ...
          if (isValidCanvasData(parsedData)) {
            setData(sanitizeCanvasData(parsedData))
      ...
      if (savedConversation) {
        try {
          ...
          if (isValidConversation(parsedConversation)) {
            ...
            setConversation(parsedConversation.slice(-50))
          } else {
      ...
      if (savedModel && typeof savedModel === 'string' && savedModel.length < 100) {
        setSelectedModel(savedModel)
      }

      if (savedWelcome) {
        try {
          setHasShownWelcome(JSON.parse(savedWelcome) === true)
        } catch {
...
Generated by: Zach Varney's Product Discovery Canvas Tool`
  ...
  // Sanitize text to prevent XSS when displaying user content
  ...
  // Validate and sanitize canvas data before sharing
  const sanitizeCanvasData = (canvasData: CanvasData): CanvasData => {
    const sanitized: CanvasData = { ...defaultData }
    for (const key of Object.keys(canvasData) as (keyof CanvasData)[]) {
      const value = canvasData[key]
  ...
  const shareCanvas = () => {
    ...
    if (url.toString().length > 8000) {
      alert('Canvas data is too large to share via URL. Please export to text instead.')
      return
    }
                  ...
                  >
                    ...
                    <div className={`p-4 rounded-lg ${
                      ...
                      <p className="text-sm whitespace-pre-wrap leading-relaxed">{
                        // Content is already text, React escapes it, but we limit length for display
